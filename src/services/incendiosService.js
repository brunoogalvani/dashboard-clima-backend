const axios = require("axios");
  ...
  async function baixarCsvFirms() {
    const cached = cache.get("firms:csv");
    if (cached) {
      console.log("[CACHE HIT] firms:csv");
      return cached;
    }
    ...
    for (const url of FIRMS_CSV_URLS) {
      try {
        ...
        if (r.status === 200 && r.data) {
          cache.set("firms:csv", r.data, 1800); // 30min — NASA atualiza a cada ~3h
          return r.data;
        }
      } catch (err) {
        lastError = err;
  ...
  exports.buscarIncendiosPorCidade = async (cidade) => {
    try {
      const cacheKey = `incendios:${cidade.toString().toLowerCase().trim()}`;
      ...
      cache.set(cacheKey, resultado, 900); // 15min
      return resultado;
    } catch (err) {
