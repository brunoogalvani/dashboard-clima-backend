...
async function buscarDadosCompletos(cidade) {
  ...
  const qualidade = resultados[1].status === "fulfilled" ? resultados[1].value : { erro: resultados[1].reason?.message || "Erro ao buscar qualidade do ar" };
  const incendios = resultados[2].status === "fulfilled" ? resultados[2].value : { erro: resultados[2].reason?.message || "Erro ao buscar incêndios" };
